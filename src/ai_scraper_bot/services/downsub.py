...
class DownSubTranscriptError(RuntimeError):
    pass


class DownSubTranscriptService:
    def __init__(
        self,
    ...
    async def fetch_transcript(self, youtube_url: str) -> str | None:
        try:
            ...
            raise DownSubTranscriptError(
                "Playwright is not installed. Install project requirements and run `playwright install chromium` to enable DownSub fallback."
            ) from exc
        ...
        async def _run() -> str | None:
            async with async_playwright() as playwright:
                ...
                try:
                    ...
                    direct_url = f"{self.base_url}turn/{encoded}"
                    await page.goto(
                        direct_url,
                        wait_until="domcontentloaded",
                        timeout=min(self.timeout_seconds, 30) * 1000,
                    )
                    await page.wait_for_timeout(1200)
                    transcript = await self._poll_for_transcript(page, base_url=self.base_url)
                    if transcript:
                        return transcript
                    raise DownSubTranscriptError(
                        "DownSub opened the direct result page, but no usable transcript was found."
                    )
                finally:
        ...
        try:
            ...
            raise DownSubTranscriptError("DownSub fallback timed out.") from exc
        except DownSubTranscriptError:
            raise
        except Exception as exc:
            logger.warning("DownSub Playwright fallback failed unexpectedly: %s", exc)
            raise DownSubTranscriptError(f"DownSub Playwright fallback failed unexpectedly: {exc}") from exc
    ...
    async def _poll_for_transcript(self, page, *, base_url: str) -> str | None:
        ...
        raise DownSubTranscriptError("DownSub loaded, but no transcript or subtitle download controls produced usable text.")
...
def _extract_transcript_from_text(page_text: str) -> str:
    ...
    for line in lines:
        lowered = line.lower()
        if any(marker in lowered for marker in ("download", "language", "subtitle downloader", "copyright", "privacy policy")):
            continue
        useful.append(line)
    joined = "\n".join(useful)
    return _normalize_downloaded_transcript(joined)
