...
This guide covers common problems you might run into while setting up or running the Discord bot version of the AI Website Scraper + Summarizer.
...
**Symptom:** `ModuleNotFoundError` for packages like `discord`, `litellm`, `fastapi`, etc. even though you installed them.
...
**Symptom:** Running the bot fails because Python cannot find the project package.
...
This is the correct run command. Do not try to run `python src/ai_scraper_bot/main.py` directly.
...
### `discord.py` fails to install

**Symptom:** `pip install discord.py` fails or the `discord` module cannot be imported.

**Fix:**

- Make sure you are installing `discord.py`, not `discord` (they are different packages).
- The correct package is already in `requirements.txt`.
- If you have both installed, uninstall both and reinstall:
...
### `PyNaCl` fails to install

**Symptom:** Build errors when installing `PyNaCl`. This package is needed for Discord voice support.
...
If you do not need voice features, the bot will still work without `PyNaCl`, but you may see a warning on startup.
...
**Symptom:** The bot starts but nothing works. No API key is loaded. Warnings about missing configuration.
...
**Symptom:** The terminal shows a warning: `No AI model configured. Set TEXT_AI_MODEL and TEXT_AI_API_KEY in .env`. Summaries fail.
...
**Symptom:** The bot crashes on startup with `RuntimeError: DISCORD_BOT_TOKEN is missing from .env`.

**Fix:** Add your Discord bot token to `.env`:
...
### Discord DNS preflight failed

**Symptom:** `RuntimeError: Discord DNS preflight failed. The current Terminal/network path could not resolve: gateway.discord.gg, discord.com`

**Fix:** This means your machine cannot reach Discord's servers. Possible causes:

- No internet connection.
- A VPN, proxy, or firewall is blocking Discord domains.
- DNS resolver issues.

Try:

1. Check your internet: `ping google.com`
2. Check Discord specifically: `nslookup discord.com`
3. If behind a VPN, try disconnecting it.
4. If on a corporate/school network, Discord may be blocked.

---

### Bot starts but does not respond to messages

**Symptom:** The bot shows as online in Discord but ignores all messages.

**Fix:**

1. **Message Content Intent not enabled:** Go to the Discord Developer Portal → your application → Bot → enable `Message Content Intent`. This is required for the bot to read message content.

2. **Bot not in the channel:** Make sure the bot has access to the channel where you are sending messages. Check channel permissions.

3. **`REPLY_TO_ALL_SERVER_MESSAGES` is false:** Check your `.env`:

```env
REPLY_TO_ALL_SERVER_MESSAGES=true
```

4. **Bot prefix issue:** The default is `!summarize`, but the bot is configured with an empty command prefix and responds to all messages when `REPLY_TO_ALL_SERVER_MESSAGES=true`. If this is set to `false`, the bot only responds to DMs and mentions.

---

### Bot responds but messages are cut off

**Symptom:** Bot responses are truncated or split into many small messages.

**Fix:** Discord has a 2000-character message limit. The bot splits long responses into chunks controlled by:

```env
MESSAGE_CHUNK_SIZE=1900
```

This is normal behavior. The value is set below 2000 to leave room for formatting. If responses are being split oddly, do not increase this value above 1950.
...
- Your `DISCORD_BOT_TOKEN` is invalid, expired, or was copied incorrectly.
- Go to the Discord Developer Portal and regenerate the token.
- Make sure there are no extra spaces, quotes, or newlines around the token in `.env`.

---

### Bot cannot send messages in a channel

**Symptom:** The bot is in the server but errors occur when trying to reply. Permission errors in the log.

**Fix:** Make sure the bot has these permissions in the server:

- View Channels
- Send Messages
- Read Message History
- Attach Files (needed for some responses)

You can configure these in:

1. Discord Developer Portal → OAuth2 → URL Generator → select permissions
2. Server Settings → Roles → your bot's role → adjust permissions

---

### Bot keeps disconnecting and reconnecting

**Symptom:** Repeated connection/disconnection messages in the terminal. `discord.errors.ConnectionClosed`.

**Fix:**

- Unstable internet connection — check your network.
- Rate limiting from Discord — if the bot is sending too many requests, Discord will throttle it.
- Multiple instances of the same bot running — only one instance can use the same token at a time. Stop any duplicate processes.

---

### Typing indicator never stops

**Symptom:** The bot shows "typing..." in Discord indefinitely for a message.

**Fix:**

- This usually means the background task is stuck or crashed silently.
- Check the terminal for error messages.
- The processing may just be slow (especially for large video files or long audio).
- Concurrency is limited by `MAX_CONCURRENT_JOBS=3`. If all slots are busy, new messages wait.

---

### Bot crashes when processing large files

**Symptom:** The bot crashes or becomes unresponsive when a user uploads a large file.
...
- Discord has its own upload limits (usually 8-25 MB depending on server boost level).
- Large files (especially video) require significant RAM and CPU for processing.
- Reduce `MAX_CONCURRENT_JOBS` to prevent multiple heavy tasks at once.

---

### Downloaded files are piling up

**Symptom:** The `Download Audio File For AI` folder keeps growing.

**Fix:** The bot has a built-in temp file sweeper controlled by:

```env
TEMP_SWEEP_HOURS=6
```

This means files older than 6 hours are automatically cleaned up. If you want faster cleanup:

```env
TEMP_SWEEP_HOURS=1
```

Or manually clean the folder:

```bash
rm -rf "Download Audio File For AI"/*
```
...
**Symptom:** Uploading a file returns an error or the bot ignores the file.
...
**Symptom:** A PDF file is uploaded but the extracted text is empty or very short.
...
### The bot is very slow on first request
...
### Multiple users overwhelm the bot

**Symptom:** The bot becomes slow or unresponsive when many users send requests at the same time.

**Fix:** The bot uses a semaphore to limit concurrent processing:

```env
MAX_CONCURRENT_JOBS=3
```

Messages beyond this limit will wait until a slot is free. If the bot is consistently overwhelmed:

- Lower the concurrent limit to prevent memory exhaustion.
- Use cloud transcription to offload heavy audio work.
- Use a faster/cheaper AI model for non-critical tasks.
...
**Symptom:** The DNS preflight check fails, or the bot cannot connect to Discord.

**Fix:**

- Check your internet connection.
- If behind a VPN, try disconnecting.
- Some networks (corporate, school) block Discord.
- Try: `nslookup gateway.discord.gg`
- If DNS resolution fails, try switching to a public DNS (e.g. 8.8.8.8 or 1.1.1.1).
